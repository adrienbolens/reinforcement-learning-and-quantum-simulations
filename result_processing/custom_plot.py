...
for i in range(len(reward_scatter)):
    ax.scatter(x_scatter, reward_scatter[i],
               c='k', alpha=0.05, marker='.', s=1)
ax.plot(x, reward_mean, label='mean fidelity')
#  ax.fill_between(x, y1, y2, alpha=0.3,
#                  label=r'$\pm$ sample $\sigma$'
#                  + f' (#samples = {n_arrays})')
ax.axhline(max_reward, label=f'max fidelity = {max_reward:.2f}', c='r')
if params['system_class'] == 'LongRangeIsing':
    ax.axhline(initial_reward,
               label=f'Trotter fidelity = {initial_reward:.2f}',
               c='r', linestyle='--')
#  ax.set_ylim([0, 1.01])
ax.set_ylim([0, 1.0])
ax.set_xlim([0, 1e5])
ax.set_xlabel('training time (episode)', fontsize=16)
ax.set_ylabel('fidelity', fontsize=16)
handles, labels = ax.get_legend_handles_labels()
order = [1, 2, 3, 0]
handles, labels = zip(*[(handles[i], labels[i]) for i in order])

ax.legend(handles, labels, fontsize=16, loc=4, bbox_to_anchor=(1.0, 0.25))
